...
export default async function (index, additionalConfigs = {}) {
    ...
    try {
        ...
        const maxTokens = await contract.availableTokens(index);

        return maxTokens;
    } catch (error) {
        console.error("[PTE] ERROR: cannot check the max token, reason: ");
        console.error(error);
        return null;
    }
}